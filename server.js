...
function generateResponse(message, customer, context, phone) {
 ...
 // תפריט ראשי - תקלה - תיקון
if (msg === '1' || msg.includes('תקלה')) {
    if (!customer) {
        console.log(`❌ אין לקוח בתקלה!`);
        return { 
            response: `שלום! 👋\n\nכדי לטפל בפנייתך אני צריכה:\n\n🏢 **שם החניון שלך**\n\n📞 039792365`, 
            stage: 'identifying' 
        };
    }
    console.log(`✅ תקלה עם לקוח: ${customer.name}`);
    return { 
...
// תפריט ראשי - נזק - תיקון
if (msg === '2' || msg.includes('נזק')) {
    if (!customer) {
        console.log(`❌ אין לקוח בנזק!`);
        return { 
            response: `שלום! 👋\n\nכדי לטפל בפנייתך אני צריכה:\n\n🏢 **שם החניון שלך**\n\n📞 039792365`, 
            stage: 'identifying' 
        };
    }
    console.log(`✅ נזק עם לקוח: ${customer.name}`);
    return { 
...
// תפריט ראשי - הצעת מחיר - תיקון
if (msg === '3' || msg.includes('מחיר')) {
    if (!customer) {
        console.log(`❌ אין לקוח בהצעת מחיר!`);
        return { 
            response: `שלום! 👋\n\nכדי לטפל בפנייתך אני צריכה:\n\n🏢 **שם החניון שלך**\n\n📞 039792365`, 
            stage: 'identifying' 
        };
    }
    console.log(`✅ הצעת מחיר עם לקוח: ${customer.name}`);
    return { 
...
// תפריט ראשי - הדרכה - תיקון
if (msg === '4' || msg.includes('הדרכה')) {
    if (!customer) {
        console.log(`❌ אין לקוח בהדרכה!`);
        return { 
            response: `שלום! 👋\n\nכדי לטפל בפנייתך אני צריכה:\n\n🏢 **שם החניון שלך**\n\n📞 039792365`, 
            stage: 'identifying' 
        };
    }
    console.log(`✅ הדרכה עם לקוח: ${customer.name}`);
    return { 
...
app.post('/webhook/whatsapp', async (req, res) => {
    try {
        if (req.body.typeWebhook === 'incomingMessageReceived') {
            ...
            console.log(`🔍 זיהוי לפי טלפון: ${customer ? customer.name + ' מ' + customer.site : 'לא מזוהה'}`);
...
if (customer) {
    ...
    console.log(`⚠️ הוסף לזיכרון ללא לקוח - שלב: ${result.stage}`);
            ...
            console.log(`📤 תגובה נשלחה ללקוח ${customer ? customer.name : 'לא מזוהה'}: ${result.stage}`);
