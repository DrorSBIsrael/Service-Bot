...
// 🧠 מערכת חכמה לזיהוי תקלות ומתן פתרונות

// 🎯 מאגר ידע מתקדם לתקלות נפוצות
const KNOWLEDGE_BASE = {
    // תקלות אשראי
    creditIssues: {
        keywords: ['אשראי', 'כרטיס', 'תשלום', 'לא עובד', 'לא קורא', 'דחה', 'error'],
        responses: {
            'אשראי לא תקין': {
                diagnosis: 'בעיה בקורא אשראי ביחידה',
                solution: [
                    'בדוק שהכרטיס נקי ולא שרוט',
                    'נסה להכניס את הכרטיס לאט יותר',
                    'בדוק שיש רשת סלולרית ביחידה',
                    'אם עדיין לא עובד - צריך איפוס יחידה'
                ],
                urgency: 'high',
                nextStep: 'אם לא עזר - אפתח קריאה לטכנאי'
            },
            'כרטיס אשראי נדחה': {
                diagnosis: 'בעיה בתקשורת עם חברת האשראי',
                solution: [
                    'בדוק שיש אות סלולרי ביחידה',
                    'נסה כרטיס אחר לבדיקה',
                    'עשה ריסטרט ליחידה (כפתור reset)',
                    'המתן 2-3 דקות ונסה שוב'
                ],
                urgency: 'high',
                nextStep: 'אם בעיה נמשכת - זה צריך טכנאי'
            }
        }
    },
    ...
    // תקלות מחסומים
    barrierIssues: {
        keywords: ['מחסום', 'זרוע', 'לא עולה', 'לא יורד', 'תקוע', 'לא נפתח'],
        responses: {
            'מחסום לא עולה': {
                diagnosis: 'בעיה במנוע המחסום או בחיישנים',
                solution: [
                    'בדוק שאין משהו חוסם את המחסום',
                    'לחץ על כפתור העלאה ידנית',
                    'בדוק שהחשמל מחובר ליחידה',
                    'אם יש צליל מוזר - עצור מיד'
                ],
                urgency: 'critical',
                nextStep: 'מחסום תקוע זה דחוף - שולח טכנאי'
            }
        }
    },
    ...
    // בעיות כלליות
    generalIssues: {
        keywords: ['לא עובד', 'תקלה', 'בעיה', 'שבור', 'לא דולק'],
        responses: {
            'לא עובד': {
                diagnosis: 'בעיה כללית ביחידה',
                solution: [
                    'בדוק שהחשמל מחובר',
                    'בדוק שיש נורות דולקות ביחידה',
                    'עשה איפוס ליחידה (כפתור reset)',
                    'המתן 30 שניות אחרי האיפוס'
                ],
                urgency: 'medium',
                nextStep: 'אם לא עזר - צריך בדיקת טכנאי'
            }
        }
    }
};

// 🔍 פונקציה חכמה לזיהוי תקלות מהקשר השיחה
function analyzeConversationForIssue(message, conversationContext, unitNumber = null) {
    const msg = message.toLowerCase();
    
    // אם יש מספר יחידה מההקשר
    let detectedUnit = unitNumber;
    if (!detectedUnit && conversationContext) {
        // חפש יחידה בהיסטוריה
        const history = conversationContext.messageHistory || [];
        for (let historyMsg of history) {
            const unitMatch = historyMsg.message.match(/\b(10[0-9]|20[0-9]|30[0-9]|60[0-9])\b/);
            if (unitMatch) {
    ...
    // בדוק תקלות אשראי
    if (KNOWLEDGE_BASE.creditIssues.keywords.some(keyword => msg.includes(keyword))) {
        issueCategory = 'creditIssues';
        
        if (msg.includes('לא תקין') || msg.includes('לא עובד')) {
            specificIssue = 'אשראי לא תקין';
        } else if (msg.includes('נדחה') || msg.includes('דחה')) {
            specificIssue = 'כרטיס אשראי נדחה';
        }
    }
    
    // בדוק תקלות מחסום
    else if (KNOWLEDGE_BASE.barrierIssues.keywords.some(keyword => msg.includes(keyword))) {
        issueCategory = 'barrierIssues';
        
        if (msg.includes('לא עולה') || msg.includes('לא נפתח')) {
            specificIssue = 'מחסום לא עולה';
        }
    }
    
    // בדוק בעיות כלליות
    else if (KNOWLEDGE_BASE.generalIssues.keywords.some(keyword => msg.includes(keyword))) {
        issueCategory = 'generalIssues';
        specificIssue = 'לא עובד';
    }
    ...
    return {
        ...
        issue: specificIssue,
        unit: detectedUnit,
        hasKnownSolution: issueCategory && specificIssue
    };
}

// 🛠️ פונקציה לייצור תגובה חכמה מבוססת ידע
function generateSmartResponse(issueAnalysis, customerData, conversationContext) {
    const { category, issue, unit, hasKnownSolution } = issueAnalysis;
    
    if (!hasKnownSolution) {
        return null; // תן ל-fallback הרגיל לטפל
    }
    
    const issueData = KNOWLEDGE_BASE[category].responses[issue];
    const customerName = customerData ? customerData.name : 'לקוח';
    const unitInfo = unit ? `יחידה ${unit}` : 'היחידה';
    
    let response = `שלום ${customerName} 👋\n\n`;
    response += `🔍 **זיהיתי את הבעיה:**\n`;
    response += `${issueData.diagnosis} ב${unitInfo}\n\n`;
    
    response += `🛠️ **בואו ננסה לפתור:**\n`;
    issueData.solution.forEach((step, index) => {
        response += `${index + 1}️⃣ ${step}\n`;
    });
    
    response += `\n`;
    
    // הוסף התראה לפי דחיפות
    if (issueData.urgency === 'critical') {
        response += `🚨 **זה דחוף!** ${issueData.nextStep}\n`;
    } else if (issueData.urgency === 'high') {
        response += `⚠️ ${issueData.nextStep}\n`;
    } else {
        response += `💡 ${issueData.nextStep}\n`;
    }
...
// 🧠 פונקציה משופרת עם חכמה מלאכותית מקומית
function generateIntelligentFallbackV2(message, customerData, conversationContext, customerName) {
    console.log('🧠 Fallback V2 - חכם מתקדם פעיל');
    
    // קודם בדוק בחירות רגילות
    const choice = analyzeCustomerChoice(message, conversationContext);
    
    if (choice) {
        console.log('✅ זוהתה בחירה רגילה:', choice.type);
        ...
        if (customerData) {
            ...
            switch(choice.type) {
                ...
                case 'unit_response':
                    // כאן נבדוק אם יש פתרון חכם
                    const issueAnalysis = analyzeConversationForIssue(message, conversationContext, choice.unitNumber || message);
                    const smartResponse = generateSmartResponse(issueAnalysis, customerData, conversationContext);
                    
                    if (smartResponse) {
                        console.log('🎯 נמצא פתרון חכם לתקלה');
    ...
    // אם לא זוהתה בחירה רגילה - בדוק אם זו תקלה חכמה
    console.log('🔍 בודק תקלות חכמות...');
    const issueAnalysis = analyzeConversationForIssue(message, conversationContext);
    
    if (issueAnalysis.hasKnownSolution && customerData) {
        console.log('🎯 זוהתה תקלה חכמה:', issueAnalysis.issue);
        const smartResponse = generateSmartResponse(issueAnalysis, customerData, conversationContext);
        if (smartResponse) {
            return smartResponse;
        }
    ...
    // אם לא זוהה כלום ספציפי - תגובות כלליות
    if (customerData) {
...
עכשיו יש לי זיכרון מתקדם של שיחות!`;
        ...
        return generateIntelligentFallbackV2(message, customerData, conversationContext, customerName);
    }
}
